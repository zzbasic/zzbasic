...
static ASTNode* parse_do_stmt(Parser* parser);
...
static char** parse_parameter_list(Parser* parser, int* param_count);
...
static ASTNode* parse_do_stmt(Parser* parser)
{
    ...
    if (parser->current_token.type != TOKEN_EOL && 
        parser->current_token.type != TOKEN_NL)
    {
        parser_set_error(parser, "newline expected after 'do'");
        return NULL;
    }
    parser_advance(parser);  // Consome EOL/NL
    ...
    if (parser->current_token.type != TOKEN_END)
    {
        parser_set_error(parser, "'end' expected in 'do' statement");
        free_ast(body);
        return NULL;
    }
    parser_advance(parser);  // Consome 'end'
    
    // Veirifica se é 'do..end' or 'do...while'
    int is_while = 0;
    ASTNode* condition = NULL;
    
    if (parser->current_token.type == TOKEN_DO)
    {
        // do...end do (apenas escopo)
        is_while = 0;
        parser_advance(parser);  // Consome 'do'
    }
    else if (parser->current_token.type == TOKEN_WHILE)
    {
        // do...end while (loop com condição)
        is_while = 1;
        parser_advance(parser);  // Consome 'while'
        
        if (parser->current_token.type != TOKEN_LPAREN)
        {
            parser_set_error(parser, "'(' expected after 'while' in 'do' statement");
            free_ast(body);
            return NULL;
        }
        parser_advance(parser);  // Consome '('
        
        condition = parse_logical_expr(parser);
        if (parser->has_error || !condition)
        {
            free_ast(body);
            return NULL;
        ...
        if (parser->current_token.type != TOKEN_RPAREN)
        {
            parser_set_error(parser, "')' expected after condition in 'do...while' statement");
            free_ast(body);
            free_ast(condition);
            return NULL;
        }
    ...
    {
        parser_set_error(parser, "'do' or 'while' expected after 'end' in 'do' statement");
        free_ast(body);
        return NULL;
    }
    ...
    if (parser->current_token.type != TOKEN_EOL && 
        ...
        parser->current_token.type != TOKEN_EOF)
    {
        parser_set_error(parser, "newline expected after 'end do' or 'end while'");
        free_ast(body);
        if (condition)
        {
            free_ast(condition);
    ...
    ASTNode* do_node = create_do_end_node(body, condition, is_while, line, column);
...
static ASTNode* parse_save_stmt(Parser* parser)
{
    ...
    ASTNode* filename_expr = parse_expr_stmt(parser);
    if (!filename_expr)
    {
        parser_set_error(parser, "expected filename expression in save()");
        free_ast(expr);
        return NULL;
    }
    ...
    return create_save_node(expr, filename_expr, line, column);
}


//===================================================================
// function_declaration := 'function' IDENTIFIER '(' parameter_list ')' EOL
//                             statement_list
//                         'end' 'function' EOL
//
...
static ASTNode* parse_function_declaration(Parser* parser)
{
    ...
    parser_advance(parser);  // Consome 'function'
    ...
    char function_name[VARNAME_SIZE];
    strncpy(function_name, parser->current_token.value.varname, VARNAME_SIZE - 1);
    function_name[VARNAME_SIZE - 1] = '\0';
    parser_advance(parser);  // Consome nome da função
    ...
    // Parseia lista de parâmetros
    int param_count = 0;
    char** parameters = parse_parameter_list(parser, &param_count);
    if (parser->has_error)
    {
        if (parameters) a89free(parameters);
        return NULL;
    }
    ...
    if (parser->current_token.type != TOKEN_RPAREN)
    {
        parser_set_error(parser, "expected ')' after parameter list");
        if (parameters) a89free(parameters);
        return NULL;
    }
    parser_advance(parser);  // Consome ')'
    ...
    if (parser->current_token.type != TOKEN_EOL && 
        parser->current_token.type != TOKEN_NL)
    {
        parser_set_error(parser, "newline expected after function declaration");
        if (parameters) a89free(parameters);
        return NULL;
    }
    parser_advance(parser);  // Consome EOL/NL
    
    // Parseia o corpo da função (statement list)
    ASTNode* body = parse_stmt_list(parser);
    if (parser->has_error || !body)
    {
        if (parameters) a89free(parameters);
    ...
    if (parser->current_token.type != TOKEN_END)
    {
        parser_set_error(parser, "'end' expected to close function");
        free_ast(body);
        if (parameters) a89free(parameters);
        return NULL;
    }
    ...
    {
        parser_set_error(parser, "'function' expected after 'end'");
        free_ast(body);
        if (parameters) a89free(parameters);
        return NULL;
    }
    ...
    if (parser->current_token.type != TOKEN_EOL && 
        ...
        parser->current_token.type != TOKEN_EOF)
    {
        parser_set_error(parser, "newline expected after 'end function'");
        free_ast(body);
        if (parameters) a89free(parameters);
        return NULL;
    }
    ...
    // Cria o nó da função
    ASTNode* func_node = create_function_def_node(function_name, line, column);
    
    // Adiciona os parâmetros
    for (int i = 0; i < param_count; i++)
    {
        function_def_add_parameter(func_node, parameters[i]);
        a89free(parameters[i]);
    }
    if (parameters) a89free(parameters);
    
    // Adiciona o corpo
    function_def_set_body(func_node, body);
    
    return func_node;
}

//===================================================================
// parse_parameter_list := [IDENTIFIER (',' IDENTIFIER)*]
// Retorna array de strings e preenche param_count
//===================================================================
static char** parse_parameter_list(Parser* parser, int* param_count)
{
    *param_count = 0;
    ...
    // Lista vazia (sem parâmetros)
    if (parser->current_token.type == TOKEN_RPAREN)
    {
        return NULL;
    }
    
    // Aloca espaço inicial para parâmetros
    params = A89ALLOC(sizeof(char*) * 10);  // Começa com 10
    ...
    while (1)
    {
        ...
        params[*param_count] = A89ALLOC(VARNAME_SIZE);
        strncpy(params[*param_count], parser->current_token.value.varname, VARNAME_SIZE - 1);
        params[*param_count][VARNAME_SIZE - 1] = '\0';
        (*param_count)++;
        
        parser_advance(parser);  // Consome o identificador
        
        // Verifica se tem mais parâmetros
        if (parser->current_token.type == TOKEN_COMMA)
        {
            parser_advance(parser);  // Consome ','
            continue;
...
static ASTNode* parse_return_statement(Parser* parser)
{
    ...
    parser_advance(parser);  // Consome 'return'
    
    ASTNode* expression = NULL;
    
    // Verifica se tem uma expressão (não é EOL/NL/EOF)
    if (parser->current_token.type != TOKEN_EOL &&
        parser->current_token.type != TOKEN_NL &&
        parser->current_token.type != TOKEN_EOF)
    {
        expression = parse_logical_expr(parser);
        if (parser->has_error)
        {
            return NULL;
        }
    ...
    if (parser->current_token.type != TOKEN_EOL && 
        ...
        parser->current_token.type != TOKEN_EOF)
    {
        parser_set_error(parser, "newline expected after 'return' statement");
        if (expression) free_ast(expression);
        return NULL;
    }
    
    return create_return_node(expression, line, column);
}
...
static ASTNode* parse_term(Parser* parser) {
           ...
           parser->current_token.type == TOKEN_PERCENT)
    {
        ...
        else if(parser->current_token.type == TOKEN_SLASH)
        {
            op = '/';
        }
        else  
...
static ASTNode* parse_atom(Parser* parser)
{
    ...
    switch (token.type)
    {
        ...
        case TOKEN_TEXT:
        {
            ASTNode* node = create_text_node(parser->current_token.text_content,
                                              parser->current_token.line,
                                              parser->current_token.column);
            
            // Liberar memória de current_token.text_content
            text_content_free(&parser->current_token);
