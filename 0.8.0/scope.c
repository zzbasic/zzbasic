...
ScopeStack* scope_stack_create(void)
{
    ...
    scope_stack->current_scope->string_pool = string_pool_create();
...
void scope_stack_destroy(ScopeStack* scope_stack)
{
    ...
    while (scope)
    {
        ...
        if (scope->string_pool)  
        {
           string_pool_free(scope->string_pool);
        }
...
void scope_push(ScopeStack* scope_stack)
{
    ...
    new_scope->string_pool = string_pool_create();
    
    if (!new_scope->symbols)
    {
        text_pool_free(new_scope->text_pool);
        string_pool_free(new_scope->string_pool);
        a89free(new_scope);
...
void scope_pop(ScopeStack* scope_stack)
{
    ...
    // Libera o pool de String, se houver  
    if (old_scope->string_pool)
    {
       string_pool_free(old_scope->string_pool);
    }
...
void scope_stack_add_text(ScopeStack* stack, Text* text)
{
    ...
    if (!current_scope || !current_scope->text_pool)
    {
        return;
    }
...
// Adiciona string ao pool do escopo atual
void scope_stack_add_string(ScopeStack* stack, char* string)
{
    if (!stack || !string) return;
    
    // Pega o escopo atual 
    Scope* current_scope = stack->current_scope;

    if (!current_scope || !current_scope->string_pool) return;

    // Adiciona ao pool do escopo
    string_pool_add(current_scope->string_pool, string);
}
