...
14. [Escopos e Variáveis Locais](#escopos-e-variáveis-locais)
15. [Resumo dos recursos](#resumo-dos-recursos)
...
- Crescimento automático (fator 1.5x)
- Leitura/escrita de arquivo
- Liberação automática ao sair do escopo
- Rastreamento de alocações com Object Tracker
...
- Escopo local dentro de blocos (IF, WHILE, FOR)
...
**Variáveis de Text:**
```basic
let texto = text("Conteúdo")
let arquivo = load("dados.txt")
```
...
- Na v0.8.0 suportam números e strings
- Índice começa em 0
...
**Descrição:** Extrai parte de uma string.
...
**Descrição:** Converte string para maiúsculas.
...
**Descrição:** Converte string para minúsculas.
...
**Descrição:** Remove espaços das extremidades.
...
**Descrição:** Divide string em array.
...
**Descrição:** Junta array em string.
...
### NOVO v0.8.0: Suporte a Funções Customizadas

**Descrição:** Permite que o usuário defina suas próprias funções.
...
**Características:**
- Nome da função: Segue regras de identificador
- Parâmetros: Zero ou mais, separados por vírgula
- Corpo: Uma ou mais declarações
- `return`: Retorna valor (opcional)
- `end function`: Obrigatório
- Suporta recursão
- Escopo local para parâmetros e variáveis internas
...
**Descrição:** Funções criam escopo local para parâmetros e variáveis.
...
1. **Tipagem:** Funções podem retornar qualquer tipo
2. **Recursão:** Suportada (limite depende da stack)
3. **Chamadas Aninhadas:** Funções podem chamar outras funções
4. **Escopo:** Parâmetros e variáveis locais são isolados
5. **Acesso Global:** Funções acessam variáveis globais
...
OBSERVAÇÃO: Na v0.8.0 apenas exibe mensagem
...
Quando um TEXT sai do escopo, a memória é liberada automaticamente. Isso foi implementado com o sistema de escopos locais.

**5. Rastreamento de Alocações**

O módulo **Object Tracker** permite rastrear todas as alocações de Text durante o desenvolvimento:
- Visualizar quais Text estão alocados
- Verificar quando são liberados
- Detectar memory leaks
- Debugar problemas de escopo

**6. Funções para manipulação do tipo Text**
- `load(<pathfile>)` - Carrega arquivo como Text
- `save(Text, pathfile)` - Salva Text em arquivo
- `text(<string>)` - Cria novo Text a partir de string
...
# Escopos locais
if (true) then
    let local_text = text("Texto local")
    print local_text nl
    # local_text é liberado automaticamente aqui
end if
```

---

## Escopos e Variáveis Locais

### Suporte a Escopos Locais

**Descrição:** Variáveis criadas dentro de blocos (IF, WHILE, FOR, FUNCTION) agora têm escopo local.

**Características:**
- Variáveis globais: Criadas fora de qualquer bloco
- Variáveis locais: Criadas dentro de blocos
- Liberação automática: Ao sair do bloco
...
if (true) then
    let local = "Local"
    print local nl
    # local é liberado aqui
end if

# Erro: local não existe mais
# print local nl  # ❌ Erro: unknown 'local' variable

print global nl  # ✅ Funciona
```
...
if (true) then
    ...
    print x nl  # Volta a "if local"
end if

print x nl  # Volta a "global"
```

### Liberação de Text ao Sair do Escopo
...
if(true) do
    ...
    print local_text nl
    # local_text é liberado aqui
end if

print global_text nl
# global_text será liberado ao final do programa
```
...
| Operador % (Módulo) | ✅ Completo | 0.8.0 |
...
| DO...END | ✅ Completo | 0.8.0 |
| Operadores de Comparação | ✅ Completo | 0.2.0 |
| Comparação de Strings | ✅ Completo | 0.8.0 |
| Operadores Lógicos | ✅ Completo | 0.3.0 |
| Arrays Tipados | ✅ Completo | 0.8.0 |
| Funções Built-in | ✅ Completo | 0.5.0 |
| Funções de String | ✅ Completo | 0.8.0 |
| **Funções Definidas pelo Usuário** | **✅ Novo** | **0.8.0** |
| Importação de Módulos | ✅ Iniciado | 0.5.0 |
| Load/Save | ✅ Completo | 0.6.0 |
| Tipo TEXT | ✅ Completo | 0.7.0 |
| Escopos Locais | ✅ Completo | 0.7.0 |
| Object Tracker | ✅ Completo | 0.7.0 |
| Liberação Automática | ✅ Completo | 0.7.0 |
...
7. Object Tracker: máximo 1000 objetos rastreados simultaneamente
8. Campo info do Object Tracker: máximo 128 caracteres
...
1. **Funções Definidas pelo Usuário**
   - Criação de funções com `function nome(params) ... end function`
   - Comando `return` para retornar valores
   - Suporte a parâmetros (0 ou mais)
   - Escopo local para parâmetros e variáveis internas
   - Chamada de função como expressão (`let x = soma(3, 5)`)
   - Funções que chamam outras funções
   - Suporte a recursão

2. **Operador % (Módulo)**
   - Operador para cálculo de resto de divisão
   - Suporta números inteiros e decimais

3. **Comparação de Strings**
   - Operadores de comparação funcionam com strings
   - Ordem lexicográfica (case-sensitive)

4. **Funções de String**
   - `substr()`, `upper()`, `lower()`, `trim()`
   - `split()`, `join()` para manipulação avançada

5. **Arrays Tipados**
   - Arrays suportam números e strings
   - `array()` com tipagem automática

6. **DO...END**
   - Estrutura `do...end` para escopo isolado
   - `do...end while` para loop com verificação no final
