...
// Estrutura para função definida pelo usuário
typedef struct UserFunction
{
    char name[VARNAME_SIZE];       // Nome da função
    char** parameters;             // Array de nomes dos parâmetros
    int param_count;               // Número de parâmetros
    ASTNode* body;                 // Corpo da função (AST)
    int scope_level;               // Nível de escopo onde foi definida
} UserFunction;
...
void symbol_table_destroy(SymbolTable* table)
{
    ...
    while (current)
    {
        ...
        if (current->type == SYM_FUNCTION && current->value.func_value)
        {
            // Libera os parâmetros
            for (int i = 0; i < current->value.func_value->param_count; i++)
            {
                if (current->value.func_value->parameters[i])
                {
                    a89free(current->value.func_value->parameters[i]);
                }
            }
            if (current->value.func_value->parameters)
            {
                a89free(current->value.func_value->parameters);
            }
            // O corpo (ASTNode*) será liberado separadamente pelo free_ast global
            a89free(current->value.func_value);
        }
...
int symbol_table_set_function(SymbolTable* table, const char* name, 
                               UserFunction* function)
{
    if (!table || !name || !function || !is_valid_name(name)) return 0;
    ...
    if (!symbol)
    {
        ...
        symbol->value.func_value = function;
    ...
    else
    {
        ...
        {
            fprintf(stderr, "%sError: '%s' is not a function%s\n",
                    COLOR_ERROR, name, COLOR_RESET);
            return 0;
        }
        
        // Libera função antiga se existir
        if (symbol->value.func_value)
        {
            // Libera os parâmetros
            for (int i = 0; i < symbol->value.func_value->param_count; i++)
            {
                if (symbol->value.func_value->parameters[i])
                {
                    a89free(symbol->value.func_value->parameters[i]);
                }
            }
            if (symbol->value.func_value->parameters)
            {
                a89free(symbol->value.func_value->parameters);
            }
            a89free(symbol->value.func_value);
        }
...
int symbol_table_get_function(SymbolTable* table, const char* name, 
                               UserFunction** out_function)
{
    if (!table || !name || !out_function) return 0;
    
    Symbol* symbol = find_symbol(table, name);
    if (!symbol)
    {
        return 0;  // Function doesn't exist
    }
    
    if (symbol->type != SYM_FUNCTION)
    {
        return 0;  // It's not a function
    }
    
    *out_function = symbol->value.func_value;
    return 1;
}

int symbol_table_has_function(SymbolTable* table, const char* name)
{
    if (!table || !name) return 0;
    
    Symbol* symbol = find_symbol(table, name);
    return (symbol && symbol->type == SYM_FUNCTION);
}


// ============================================
// UserFunction accessor functions
// ============================================

const char* user_function_get_name(UserFunction* func)
{
    if (!func) return NULL;
    return func->name;
}

int user_function_get_param_count(UserFunction* func)
{
    if (!func) return 0;
    return func->param_count;
}

const char* user_function_get_parameter(UserFunction* func, int index)
{
    if (!func || index < 0 || index >= func->param_count) return NULL;
    return func->parameters[index];
}

ASTNode* user_function_get_body(UserFunction* func)
{
    if (!func) return NULL;
    return func->body;
}
