...
static char lexer_peek_previous(Lexer* lexer);
                                ...
                                ...)
{
    ...
    fprintf(stderr, "%s[%d:%d] Lexer error:%s %s\n",
            COLOR_ERROR,
            line,
            column,
            COLOR_RESET,
    ...
    // Exibe a linha com uma seta apontando para o erro
    display_error_location(lexer->source,
                           lexer->source_size, 
                           line,
                           column);
    ...
    {
        exit(1);  // Encerra o programa para arquivo .zz
    }

    // Se chegou aqui, é REPL 
    ...
    strncpy(token.token_text, message, BUFFER_SIZE - 1);
    token.token_text[BUFFER_SIZE - 1] = '\0';
    token.line = line;
    token.column = column;
...
// Retorna o caractere anterior pulando espaços, tabs, \r
static char lexer_peek_previous(Lexer* lexer)
{
    int pos = lexer->position - 1;
    
    // Pular espaços, tabs, \r para trás
    while (pos >= 0 && 
           (lexer->source[pos] == ' ' || 
            lexer->source[pos] == '\t' || 
            lexer->source[pos] == '\r'))
...
static Token lexer_read_text(Lexer* lexer)
{
    int text_line = lexer->line;
    int text_column = lexer->column;

    char* buffer = (char*)A89ALLOC(TEXT_SIZE);
    if (!buffer)
    {
        return lexer_report_error(lexer, text_line, text_column,
            "Memory allocation failed for text content");
    }
    
    int i = 0;
    
    // Consome as 3 " iniciais
    lexer_advance(lexer);  // primeira "
    lexer_advance(lexer);  // segunda "
    lexer_advance(lexer);  // terceira "
    
    // Lê até encontrar """ de fechamento
    while (lexer->current_char != '\0')
    {
        ...
        if (i >= TEXT_SIZE - 1)
        {
            a89free(buffer);
            return lexer_report_error(lexer, text_line, text_column,
                "TEXT exceeds maximum size of %d bytes. "
                "Maximum allowed: %d KB (~8 pages)",
    ...
    token.line = text_line;
    token.column = text_column;
    strcpy(token.token_text, "\"\"\"");  // Marcar como multiline
    token.text_content = buffer;  // Transferir buffer para token
...
Token lexer_get_next_token(Lexer* lexer)
{
    ...
    if (lexer->line_length >= PROGRAM_LINE_SIZE)
    {
        return lexer_report_error(lexer,
                                  lexer->line,
                                  1,
                                  "Line %d exceeds %d character limit",
                                  lexer->line, PROGRAM_LINE_SIZE);
    }
    ...
    while (1)  
    {
        ...
        //  COMENTARIO DE VARIAS LINHAS OU TEXT MULTILINE
        if (lexer->current_char == '"' &&
            lexer_peek_next(lexer) == '"' &&
            lexer->position + 2 < lexer->source_size &&
            lexer->source[lexer->position + 2] == '"')
        {
            // Verificar se é TEXT multiline ou comentário
            char prev = lexer_peek_previous(lexer);
            
            // Se o caractere anterior é '=' ou '(' ou ',' → é TEXT multiline
            if (prev == '=' || prev == '(' || prev == ',')
            {
                // TEXT multiline
                return lexer_read_text(lexer);
            }
            else
            {
...
// Liberar memória de token->text_content
void text_content_free(Token* token)
{
    if (!token) return;
    
    if (token->type == TOKEN_TEXT && token->text_content)
    {
        a89free(token->text_content);
        token->text_content = NULL;
    }
}
