...
typedef struct
{
    ...
    ASTNode* filename_expr;      // Nome do arquivo (pode ser STRING, VARIÁVEL, etc)
} SaveStatementData;
...
typedef struct
{
    ASTNode* body;              // Statements do do...end
    ASTNode* condition;         // Condição do while (pode ser NULL para do...end do)
    int is_while;               // 1 = do...while, 0 = do...end do
} DoEndStatementData;

typedef struct FunctionDefData
{
    char function_name[VARNAME_SIZE];   // Nome da função
    char** parameters;                  // Array de nomes dos parâmetros
    int param_count;                    // Número de parâmetros
    ASTNode* body;                      // Corpo da função (statements)
...
typedef struct ASTNode
{
    ...
    union
    {
        ...
        DoEndStatementData      do_end_stmt;
        FunctionDefData         function_def;    
        ReturnData              return_stmt;     
    } data;
