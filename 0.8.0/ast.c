...
ASTNode* create_text_node(const char* value, int line, int column)
{
    ASTNode* node = create_node(NODE_TEXT, line, column);
    
    if (value)
    {
        // Alocar na heap
        node->data.text.value = (char*)A89ALLOC(strlen(value) + 1);
        if (node->data.text.value)
        {
            strcpy(node->data.text.value, value);
        }
    }
    else
    {
        node->data.text.value = NULL;
    }
...
ASTNode* create_do_end_node(ASTNode* body, ASTNode* condition, int is_while,
                            int line, int column)
{
    ASTNode* node = A89ALLOC(sizeof(ASTNode));
    node->type = NODE_DO;
    node->line = line;
    node->column = column;
    node->data.do_end_stmt.body = body;
    node->data.do_end_stmt.condition = condition;
    node->data.do_end_stmt.is_while = is_while;    
    return node;
}
...
ASTNode* create_save_node(ASTNode* expression, ASTNode* filename_expr, int line, int column)
{
    ASTNode* node = create_node(NODE_SAVE, line, column);
    node->line = line;
    node->column = column;
    node->data.save_stmt.expression = expression;
    node->data.save_stmt.filename_expr = filename_expr;
    return node;
}
...
ASTNode* create_function_def_node(const char* function_name, int line, int column)
{
    ASTNode* node = create_node(NODE_FUNCTION_DEF, line, column);
    
    strncpy(node->data.function_def.function_name, function_name, VARNAME_SIZE - 1);
    node->data.function_def.function_name[VARNAME_SIZE - 1] = '\0';
    
    node->data.function_def.parameters = NULL;
    node->data.function_def.param_count = 0;
    node->data.function_def.body = NULL;
    node->data.function_def.scope_level = 0;
    
    return node;
}

void function_def_add_parameter(ASTNode* node, const char* param_name)
{
    if (!node || node->type != NODE_FUNCTION_DEF || !param_name) return;
    
    FunctionDefData* func = &node->data.function_def;
    
    // Realoca array de parâmetros
    char** new_params = A89REALLOC(func->parameters, (func->param_count + 1) * sizeof(char*) );
    if (!new_params) return;
    
    func->parameters = new_params;
    
    // Aloca e copia o nome do parâmetro
    func->parameters[func->param_count] = A89ALLOC(VARNAME_SIZE);
    strncpy(func->parameters[func->param_count], param_name, VARNAME_SIZE - 1);
    func->parameters[func->param_count][VARNAME_SIZE - 1] = '\0';
    
    func->param_count++;
}

void function_def_set_body(ASTNode* node, ASTNode* body)
{
    if (!node || node->type != NODE_FUNCTION_DEF) return;
...
void free_ast(ASTNode* node)
{
    ...
    switch (node->type) {
        ...
        case NODE_DO:
            if (node->data.do_end_stmt.body)
            {
                free_ast(node->data.do_end_stmt.body);
            }
            if (node->data.do_end_stmt.condition)
            {
                free_ast(node->data.do_end_stmt.condition);
            }
            break;
        ...
        case NODE_SAVE:
            if (node->data.save_stmt.expression)
            {
                ...
                free_ast(node->data.save_stmt.filename_expr);
            }
            break;
        ...
        case NODE_FUNCTION_DEF:
        {
            ...
            for (int i = 0; i < node->data.function_def.param_count; i++)
            {
                ...
                {
                    a89free(node->data.function_def.parameters[i]);
                }
            }
            if (node->data.function_def.parameters)
            {
                a89free(node->data.function_def.parameters);
            }
            // Libera o corpo
...
void print_ast(ASTNode* node, int indent)
{
    ...
    switch (node->type)
    {
        ...
        case NODE_DO:
            printf("%s (do...%s)\n", "NODE_DO_END", 
                   node->data.do_end_stmt.is_while ? "while" : "end do");
            printf("%s  Body:\n", indent + 1);
            if (node->data.do_end_stmt.body)
            {
                print_ast(node->data.do_end_stmt.body, indent + 2);
            }
            if (node->data.do_end_stmt.condition)
            {
                printf("%s  Condition:\n");
                print_ast(node->data.do_end_stmt.condition, indent + 2);
            }
            break;
        ...
        case NODE_SAVE:
            ...
            printf("Filename:\n");
            print_ast(node->data.save_stmt.filename_expr, indent + 1);
            break;
        ...
        case NODE_FUNCTION_DEF:
            printf("FUNCTION_DEF: %s(", node->data.function_def.function_name);
            for (int i = 0; i < node->data.function_def.param_count; i++)
            {
                if (i > 0) printf(", ");
